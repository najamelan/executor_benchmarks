...
fn ring( c: &mut Criterion )
{
	...
	for nodes in [10, 100, 200].iter()
	{
		...
		group.bench_function( format!( "ThreadPool spawn_handle_os {}", &nodes ), |b|
		{
			let pool = ThreadPool::new().expect( "create threadpool" );
			...
			b.iter( move ||
			{
				...
				let mut ring = HandleRingOs::new( *nodes );
		...
		group.bench_function( format!( "tokio::runtime::Runtime (basic_scheduler) {}", &nodes ), |b|
		{
		...
		group.bench_function( format!( "TokioTp SpawnHandleOs {}", &nodes ), |b|
		{
			let mut pool = TokioTp::try_from( &mut Builder::new() ).expect( "build tokio threadpool" );
		...
		group.bench_function( format!( "AsyncStd SpawnHandleOs {}", &nodes ), |b|
		{
			let pool = AsyncStd::default();
